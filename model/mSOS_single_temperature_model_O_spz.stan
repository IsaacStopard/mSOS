...
functions{
  ...
  real log_likelihood_survival(real delta, int infection_status, int censored, real baseline_hazard, real cox_u, real cox_i, real S_t_i, real S_t_u){
    real log_likelihood;
    //if(censored == 1){
    //    if(infection_status == 1){
    //      log_likelihood = log((delta * baseline_hazard * S_t_i * cox_i) + 
    //                                      ((1-delta) * baseline_hazard * S_t_u * cox_u));
    //    } else if(infection_status < 1){
    //      log_likelihood = log(baseline_hazard * S_t_u * cox_u);
    //    }
    //} else if(censored < 1){
    //    if(infection_status == 1){
    ...
    if(censored == 1){
        if(infection_status == 1){
          log_likelihood = log_sum_exp((log(delta) + baseline_hazard + cox_i + S_t_i),(log(1-delta) + baseline_hazard + cox_u + S_t_u));
        } else if(infection_status < 1){
          log_likelihood = baseline_hazard + cox_u + S_t_u;
        }
    } else if(censored < 1){
        if(infection_status == 1){
