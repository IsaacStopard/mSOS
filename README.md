...
This GitHub repository provides all code necessary to run the multiscale model of sporogony (mSOS) applied in "Estimating the extrinsic incubation period of malaria using a mechanistic model of sporogony".
...
:warning: Please note that the Stan outputs are not saved on the Github repository, so run_mSOS.R will need to be run before attempting to visualise results.
...
:warning: This model is under development, so code may change without notice. No liability is accepted by the authors.
